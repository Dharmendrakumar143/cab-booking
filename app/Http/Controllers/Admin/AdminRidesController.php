...
class AdminRidesController extends AdminBaseController
{
    public function rides()
    {
        ...
        // paused rides
        $query = RideRequests::whereHas('booking', function ($query) use ($admin_role, $admin_id) {
            $query->where('booking_status', 'Paused');
        ...
        $paused_rides_count = $query->count();
        ...
        return view('admin.rides.index',compact('paused_rides_count','confirmed_rides','confirmed_rides_count','pending_rides_count','pre_cancelled_rides','upcoming_rides','completed_rides','completed_rides_count','pending_rides','in_progress_rides','count_in_progress_ride','cancelled_by_drivers'));
    }
   

    public function rideDetails(Request $request, $ride_id = null)
    {
        $admin = Auth::guard('admin')->user();
        $admin_id = $admin->id;
        $admin_role = $admin->roles->first()->name ?? null;
        ...
        if ($admin_role !== 'super-admin' && $admin_role !== 'admin') {
            if(isset($driver_id) && ($driver_id != $admin_id)){
                $request->session()->flash('error', 'This ride has already been accepted by someone.');
    ...
    public function acceptRideRequest(Request $request)
    {
       ...
       // Check if the admin role is "independent-contractors" and validate Stripe account ID
        ...
        if ($booking_status == 'Paused') {
            $message = "The ride has been paused.";
        }
    ...
    public function showRideRequestDriver(Request $request)
    {
        ...
        foreach ($drivers as $driver) {
            ...
            if ($driver_email) {
                Mail::to($driver_email)->send(new NewRideRequestDriverMail($booking_ride,'driver'));
            }
        }
    ...
    public function pendingRideData(Request $request)
    {
        ...
        if ($request->has('date_filter') && $request->date_filter != '') {
            ...
            $query->where('pick_up_date',$dateFilter);
        }
        ...
        }elseif($orderColumn=='pick_up_time'){
            $query->orderByRaw("STR_TO_DATE(pick_up_time, '%h:%i %p') {$orderDirection}");
        }else{
        ...
        foreach($pending_rides as $pending_ride){
            ...
            if ($pending_ride->pick_up_time) {
                $pending_ride->pick_up_time = Carbon::parse($pending_ride->pick_up_time)->format('H:i');
            }
    ...
    public function completedRideData(Request $request)
    {
        ...
        if ($request->has('date_search') && $request->date_search != '') {
            ...
            $query->where('pick_up_date',$dateSearch);
        }
        ...
        foreach($complete_rides as $complete_ride){
            ...
            if ($complete_ride->pick_up_time) {
                $complete_ride->pick_up_time = Carbon::parse($complete_ride->pick_up_time)->format('H:i');
            }
        }
    ...
    public function acceptedRideData(Request $request)
    {
        ...
        foreach($confirm_rides as $confirm_ride){
            ...
            if ($confirm_ride->pick_up_time) {
                $confirm_ride->pick_up_time = Carbon::parse($confirm_ride->pick_up_time)->format('H:i');
            }
        }
    ...
    public function pausedRideData(Request $request)
    {
        $admin = Auth::guard('admin')->user();
        $admin_id = $admin->id;
        $admin_role = $admin->roles->first()->name ?? null;
        ...
        // Pagination logic
        $totalRecords = $query->count();
        $paused_rides = $query->skip($request->start)
            ->take($request->length)
            ->with(['booking.admin', 'users','paymentStatus'])
            ->orderBy('created_at','desc')
            ->get();

        foreach($paused_rides as $paused_ride){
            $paused_ride = checkSurgePrice($paused_ride);
            
            // Format pick_up_date if available
            if ($paused_ride->pick_up_date) {
                $paused_ride->pick_up_date = Carbon::parse($paused_ride->pick_up_date)->format('m/d/Y');
            }

            if ($paused_ride->pick_up_time) {
                $paused_ride->pick_up_time = Carbon::parse($paused_ride->pick_up_time)->format('H:i');
            }
        }
