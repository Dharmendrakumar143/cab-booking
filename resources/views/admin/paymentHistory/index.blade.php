...
<div class="payment-histroy-table-box custom-table mt-5">
    <div class="passe-heading mb-2 ">
        <h4 class="color-20 fw-600">Payment History</h4>
    </div>
    <div class="payment-histroy d-flex justify-content-between align-items-center  mb-3 flex-lg-nowrap flex-wrap">
        <div class="pdf-btn-filter d-flex flex-lg-nowrap flex-wrap align-items-center">
            <a href="{{route('payment-history.export.excel')}}" class="ex-btn fw-600 text-black f7f8-bg rounded-30 py-1 px-5">Excel</a>
            <a href="{{route('payment-history.payment')}}" class="cs-btn fw-600 text-black f7f8-bg rounded-30 py-1 px-5 mx-2">CSV</a>
            <a href="{{route('export-payment-history-pdf')}}" class="pd-btn fw-600 text-black f7f8-bg rounded-30 py-1 px-5 mt-lg-0 mt-md-0 mt-3">PDF</a>
        </div>
...
<div class="payment-histroy-table custom-table mt-3 mb-3">
    <div class="table-responsive" id="payment_filter_result">
        <table class="table">
                        ...
                        <td>
                            <div class="td-delete-icon d-flex gap-3">
                                @if(Auth::guard('admin')->check()) 
                                    ...
                                    @if($user->hasRole('super-admin') || $user->hasPermissionTo('view payment details'))
                                        <a href="{{route('payment-details',['ride_id'=>$payment_status->id])}}">
                                            <img src="{{asset('assets/images/td-eye.png')}}">
                                        </a>                                                                                                                                                                                                                                                                                         
                                    @endif
                                    @if($user->hasRole('super-admin') || $user->hasPermissionTo('delete payments'))
                                        <a href="javascript:void(0);" class="delete-btn" data-ride-id="{{ $payment_status->id }}">
...
<script>
    $(document).on('click', '.delete-btn', function() {
        var rideId = $(this).data('ride-id'); // Get ride ID
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = '{{ route("payment-delete", ["ride_id" => "__ride_id__"]) }}'.replace("__ride_id__", rideId);
            }
        });
    });
        ...
        function paymentHistoryFilter(search_by){
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $('#payment_filter_result').html(response);
                },
